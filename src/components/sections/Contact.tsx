...
export function Contact() {
  ...
  async function onSubmit(values: ContactFormValues) { await submitContactForm(values, profile.whatsapp); reset(); }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
          ...
          <motion.form initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }}
            ...
            <div className="flex items-center gap-4">
              ...
              {isSubmitSuccessful && (
                <motion.p initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2 text-sm text-green-400">
                  <CheckCircle2 size={16} /> Sent! Check WhatsApp.
                </motion.p>
              )}
            </div>
